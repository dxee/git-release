...
if [[ "${CHANGELOG}" == "Y" ]]; then
  ...
  if grep -qE "^([0-9])+.([0-9])+.([0-9])+$" <<<"${NEXT_VERSION}"; then
    "${GIT_REPO_DIR}"/git-changlog/run-changelog.sh -r -o -t "${RELEASE_TAG}" && cd "${GIT_REPO_DIR}"
  else
    "${GIT_REPO_DIR}"/git-changlog/run-changelog.sh -o -t "${RELEASE_TAG}" && cd "${GIT_REPO_DIR}"
  fi
  git add .
